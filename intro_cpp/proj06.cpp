...
main( int argc, char *argv[ ] )
{
	...
	// Input buffers: kernel will read from these
	...
	// Output buffers: kernel will write to these
	...
	size_t globalWorkSize[3] = { (size_t)DATASIZE,  1, 1 };
	...
	status = clEnqueueReadBuffer( CmdQueue, dSumx4,  CL_TRUE, 0, xySize, hSumx4,  0, NULL, NULL ); // using CL_TRUE for blocking reads
	status = clEnqueueReadBuffer( CmdQueue, dSumx3,  CL_TRUE, 0, xySize, hSumx3,  0, NULL, NULL );
	status = clEnqueueReadBuffer( CmdQueue, dSumx2,  CL_TRUE, 0, xySize, hSumx2,  0, NULL, NULL );
	status = clEnqueueReadBuffer( CmdQueue, dSumx,   CL_TRUE, 0, xySize, hSumx,   0, NULL, NULL );
	status = clEnqueueReadBuffer( CmdQueue, dSumx2y, CL_TRUE, 0, xySize, hSumx2y, 0, NULL, NULL );
	status = clEnqueueReadBuffer( CmdQueue, dSumxy,  CL_TRUE, 0, xySize, hSumxy,  0, NULL, NULL );
	status = clEnqueueReadBuffer( CmdQueue, dSumy,   CL_TRUE, 0, xySize, hSumy,   0, NULL, NULL );
...
#else
	fprintf( stderr, "Array Size: %8d, Work-Group Size: %4d, MegaPoints/Sec: %10.2lf, Q,L,C: (%5.1f, %5.1f, %5.1f)\n",
	...
	clReleaseContext(       Context  ); // release the context
...
SelectOpenclDevice()
{
	...
	for( int p = 0; p < (int)numPlatforms; p++ )
	{
		...
		if( status != CL_SUCCESS && status != CL_DEVICE_NOT_FOUND) // CL_DEVICE_NOT_FOUND is OK
			fprintf(stderr, "clGetDeviceIDs failed (1)\n");
			...
			else					// holding something already -- can we do better?
			{
				...
				else										// holding a gpu -- is a better gpu available?
				{
					...
					{										// we are assuming we just found a bigger, badder nvidia or amd gpu
						if( vendor == ID_NVIDIA || vendor == ID_AMD)
						{
							bestPlatform = p;
							bestDevice = d;
							Platform = platforms[bestPlatform];
...
Determinant( float c0[3], float c1[3], float c2[3] )
{
	...
	float d01 = c0[0] * ( c1[2]*c2[1] - c1[1]*c2[2] ); // corrected determinant calculation
	float d02 = c0[0] * ( c1[1]*c2[2] - c1[2]*c2[1] ); // this seems like a bug in original code
	return d00 - c1[0] * ( c0[1]*c2[2] - c0[2]*c2[1] ) + c2[0] * ( c0[1]*c1[2] - c0[2]*c1[1] ); // Actual determinant
}
...
Solve( float A[3][3], float X[3], float B[3] )
{
	...
	float detQ =   B[0]*(A[1][1]*A[2][2] - A[2][1]*A[1][2]) - A[0][1]*(  B[1]*A[2][2] -   B[2]*A[1][2]) + A[0][2]*(  B[1]*A[2][1] -   B[2]*A[1][1]);
	float detL = A[0][0]*(  B[1]*A[2][2] -   B[2]*A[1][2]) -   B[0]*(A[1][0]*A[2][2] - A[2][0]*A[1][2]) + A[0][2]*(A[1][0]* B[2] - A[2][0]* B[1]);
	float detC = A[0][0]*(A[1][1]* B[2] - A[2][1]* B[1]) - A[0][1]*(A[1][0]* B[2] - A[2][0]* B[1]) +   B[0]*(A[1][0]*A[2][1] - A[2][0]*A[1][1]);
